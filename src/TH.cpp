...
{   
    ...
    const double Coef_eex = (2*this->Epsx-grid.dt*this->Rhoxe)
                    /(2*this->Epsx+grid.dt*this->Rhoxe);
    const double Coef_ehx = (2*grid.dt)
                    /((2*this->Epsx+grid.dt*this->Rhoxe)*grid.ds);
    const double Coef_eix = (-2*grid.dt)
                    /(2*this->Epsx+grid.dt*this->Rhoxe);

    const double Coef_eey = (2*this->Epsy-grid.dt*this->Rhoye)
                    /(2*this->Epsy+grid.dt*this->Rhoye);
    const double Coef_ehy = (- 2*grid.dt)
                    /((2*this->Epsy+grid.dt*this->Rhoye)*grid.ds);
    const double Coef_eiy = (-2*grid.dt)
                    /(2*this->Epsy+grid.dt*this->Rhoye);
...
{
    const unsigned int ux = grid.xy(x+1,y);
    const unsigned int uy = grid.xy(x,y+1);
    const unsigned int xy = grid.xy(x,y);

    const double Coef_hhz = (2*this->Muz-grid.dt*this->Rhozm)
                            /(2*this->Muz+grid.dt*this->Rhozm);
    const double Coef_hex = (2*grid.dt)
                            /((2*this->Muz+grid.dt*this->Rhozm)*grid.ds);
    const double Coef_hzm = -(2*grid.dt)
                            /(2*this->Muz+grid.dt*this->Rhozm);
...
void THGrid::update()
{
    for(int y = 0; y < this->grid.ny ; y++)
    {
        for(int x = 0; x < this->grid.nx ; x++)
        {
            ...
            if(!this->current)
            {
               this->cells[this->current][xy].updateE(this->grid,this->cells[1-this->current],this->time,x,y);
            }
            else
            {
               this->cells[this->current][xy].updateH(this->grid,this->cells[1-this->current],this->time,x,y);
            }
            time += 0.5*this->grid.dt;
            std::cout << this->cells[this->current][xy].Hz << " ";
        }
        std::cout << std::endl;
    }
